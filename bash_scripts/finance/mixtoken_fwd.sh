...
WANDB_PROJECT=specInfer python distill/train.py \
    ...
    --bf16 True \
    --output_dir $datapath/finance_mixtoken_fwd \
    --num_train_epochs 2 \
    ...
    --run_name finance_mixtoken_fwd \
    --mode offline \
    --sample_source mix_token \
    --kl_method forward

mkdir output
python distill/experiment/compare_model.py \
       --data data/gbharti_finance-alpaca_eval.json \
       --student $datapath/finance_mixtoken_fwd   > output/finance_mixtoken_acc.out