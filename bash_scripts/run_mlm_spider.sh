...
python3 -m torch.distributed.run --nproc_per_node=8 \
    ...
    --dataset_name spider \
    --source_prefix "Could you translate the following question into SQL. Please only generate SQL, don't include explanation in the answer. " \
    --is_qa True \
    --do_train \
    --do_eval \
    --num_train_epochs 3 \
    ...
    --train_target_max_length 128 \
    --val_target_max_length 128 \
    --test_target_max_length 128 \
    --output_dir /home/lanxiang/MIT/LLMs_and_TVM/specd/specNBCE-main/mlm_spider_seqlen128 2>&1 | tee logs/train/spider_t5_xl_to_small_trial_${trial}.log
