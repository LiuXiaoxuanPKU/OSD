...
class PredictorTrainer(Trainer):
    ...
    def compute_loss(self, model, inputs, return_outputs=False):
        ...
        all_predictor_labels = inputs["labels"]
        #print(all_predictor_labels.shape)
        ...
        while not all(all_eos_flag) and critical_len + iter_counter < self.args.model_max_length:
            ...
            label_idx = iter_counter * 2
            predictor_labels = all_predictor_labels[:, label_idx:label_idx+2]

            valid_label_flags = []

            for b in range(input_ids.shape[0]):
                if -1 not in predictor_labels[b, :]:
                    valid_label_flags.append(False)
                else:
                    valid_label_flags.append(True)
            ...
            # inspect whether the output contains eos
            # update the flag accordingly
            for b in range(input_ids.shape[0]):
                if all_eos_flag[b] == True:
                    continue

                new_token = input_ids[b, -1]
                print(new_token)
            ...
            predictor_labels = predictor_labels.to(predictor_values.device, dtype=float)
            ...
            for i in range(predictor):           
                ...
                loss_i = 0
                for b in range(input_ids.shape[0]):
                    if not all_eos_flag[b] and valid_label_flags[b]:
...
def preprocess(
    ...
    for i, source in enumerate(sources):
        ...
        for j, record_i in enumerate(source):
            ...
            #labels = labels[:predictor_num_heads]
    ...
    #print(all_predictor_labels)
    #all_labels = []
    #for sd_i_label in all_predictor_labels:
    #    labels = tokenizer(
    #        sd_i_label,
    #        return_tensors="pt",
    #        padding="max_length",
